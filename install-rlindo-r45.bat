...
set RTOOLS_VER=rtools45-6691-6492.exe
set RMIN=4.0
...
FOR /F "tokens=*" %%A IN (version.txt) DO (
    set LINE=%%A
    echo !LINE! | findstr /C:"Platform:" >nul
    if !ERRORLEVEL! EQU 0 (
        if not "!LINE!" == "!LINE:x86_64=!" (
...
if not "!RTOOLS_INSTALLED!"=="true" (
    ...
    if not exist %RTOOLS_VER% (
        ...
        wget.exe --no-check-certificate https://cran.r-project.org/bin/windows/Rtools/rtools45/files/%RTOOLS_VER% -q --show-progress
